...
var stemmer = (function () {
  ...
  return function (t) {
    ...
    if (
      ((x = t.substr(0, 1)),
      'y' == x && (t = x.toUpperCase() + t.substr(1)),
      ($ = /^(.+?)(ss|i)es$/),
      (p = /^(.+?)([^s])s$/),
      $.test(t)
        ? (t = t.replace($, '$1$2'))
        : p.test(t) && (t = t.replace(p, '$1$2')),
      ($ = /^(.+?)eed$/),
      (p = /^(.+?)(ed|ing)$/),
      $.test(t))
    ) {
      ...
      (l = f[1]),
        ...
        p.test(l) &&
          ((t = l),
          (p = /(at|bl|iz)$/),
          (v = new RegExp('([^aeiouylsz])\\1$')),
          (g = new RegExp('^' + a + s + '[^aeiouwxy]$')),
          p.test(t)
            ? (t += 'e')
            : v.test(t)
            ? (($ = /.$/), (t = t.replace($, '')))
    ...
    if ((($ = /^(.+?)e$/), $.test(t))) {
      ...
      (l = f[1]),
        ($ = new RegExp(c)),
        (p = new RegExp(o)),
        (v = new RegExp('^' + a + s + '[^aeiouwxy]$')),
        ($.test(l) || (p.test(l) && !v.test(l))) && (t = l);
    }
    return (
      ($ = /ll$/),
      (p = new RegExp(c)),
      $.test(t) && p.test(t) && (($ = /.$/), (t = t.replace($, ''))),
      'y' == x && (t = x.toLowerCase() + t.substr(1)),
      t
    );
  };
})();
...
export class BM25 {
  ...
  addDocument(doc) {
    ...
    this.averageDocumentLength =
      this.totalDocumentTermLength / this.totalDocuments;
  ...
  search(query) {
    ...
    for (var j = 0, nDocs = keys.length; j < nDocs; j++) {
      ...
      for (var i = 0, len = queryTerms.length; i < len; i++) {
        ...
        var denom =
          this.documents[id].terms[queryTerm].count +
          this.k1 *
            (1 -
              this.b +
              (this.b * this.documents[id].termCount) /
                this.averageDocumentLength);
